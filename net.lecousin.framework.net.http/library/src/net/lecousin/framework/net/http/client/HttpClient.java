...
public class HttpClient {
	...
	public HttpResponse send(HttpRequest req, boolean followRedirect, WorkProgress progress, int amount) throws UnknownHostException, IOException {
		...
		HttpResponse resp = new HttpResponse(in, progress, stepReceive);
		in.close();
		out.close();
		socket.close();
		if (Log.debug(this))
		...
		if (followRedirect && (resp.getStatusCode() == HttpStatus.SC_MOVED_PERMANENTLY || resp.getStatusCode() == HttpStatus.SC_MOVED_TEMPORARILY)) {
			if (progress != null)
				progress.undo(amount);
			...
			return send(req, true, progress, amount);
		}
		return resp;
	}
